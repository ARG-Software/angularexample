import { Component, ContentChild, EventEmitter, HostListener, SimpleChanges,
    Input, Output, TemplateRef, ViewChild, ViewEncapsulation, OnChanges, } from '@angular/core';
import { BaseChartComponent, LineSeriesComponent } from '@swimlane/ngx-charts';
...
export class BaseComboChartComponent extends BaseChartComponent implements OnChanges {
    ...
    @Input() public schemeType: string;
    ...
    public seriesDomain;
    public scaledAxis;
    public combinedSeries;
    public xSet;
    public filteredDomain;
    public hoveredVertical;
    ...
    public updateHoveredVertical(item): void {
        this.hoveredVertical = item.value;
        this.deactivateAll();
    }

    public updateDomain(domain): void {
    ...
    public getScaleType(values): string {
        let date = true;
        let num = true;
    ...
    public getXScaleLine(domain, width): any {
        let scale;
    ...
    public getYScaleLine(domain, height): any {
    ...
    public getXDomain(): any[] {
        return this.barData.map((d) => d.name);
    }

    public getYDomain() {
        const values = this.barData.map((d) => d.value);
    ...
    public setColors(): void {
        ...
        this.colors = new ColorHelper(this.scheme.Bar, this.schemeType, domain, this.customColors);
        this.colorsLine = new ColorHelper(this.scheme.Line, this.schemeType, domain, this.customColors);
    }
    ...
    public updateLineWidth(width): void {
        this.bandwidth = width;
    }

    public updateYAxisWidth({ width }): void {
        this.yAxisWidth = width + 20;
        this.update();
    }

    public updateXAxisHeight({ height }): void {
        this.xAxisHeight = height;
        this.update();
    }

    public onActivate(item) {

...
import { GetProductDetails, GetProductDetailsSuccess,
    SaveProductDetails, GetOperations, GetOperationsSuccess,
    GetOperationsSelectBoxSuccess, AddOperation, AddOpertionSuccess,
    RemoveOperation, RemoveOperationSuccess, GetMachineOperations,
    GetMachineOperationSuccess, AddMachineOperation, AddMachineOperationSuccess,
    RemoveMachineOperation, RemoveMachineOperationSuccess, GetMotes, GetMotesSuccess,
    GetMotesSelectBoxSuccess, AddMote, AddMoteSuccess, RemoveMote, RemoveMoteSuccess, GetSensors,
    GetSensorsSuccess, AddSensor, AddSensorSuccess, RemoveSensor, RemoveSensorSuccess, GetEdgesSelectBox,
    GetEdgesSelectBoxSuccess, GetSubcontractorsSelectBox, GetSubcontractorsSelectBoxSuccess, GetMessagesSelectBox,
    GetMessagesSelectBoxSuccess, GetMachineSelectBox, GetMachineSelectBoxSuccess, GetResumePage, FinishWizard } from '../actions/configure.actions';
import * as Factory from 'factory.ts';
import { ProductModelUI, OperationModelUI, ConfigureSelectBoxModelUI, MachineOperationModelUI,
    MoteModelUI, SensorModelUI } from '../models/configure.model';
...
describe('Configure Reducer : Product Detail State', () => {
    ...
    describe('[Configure] Save Product Details Success', () => {
        ...
        () => {
            ...
            expect(result).toEqual({...initialState , productsPage:
                {
                    ...initialState.productsPage,
                    productDetails: {...fakeCurrentProductDetails, ...fakeProductDetails }
                }
                });
        });
    });
...
describe('Configure Reducer : Operations State', () => {
    ...
    beforeEach(() => {
        ...
        mockOperationSelectBoxModelUI = Factory.Sync.makeFactory<ConfigureSelectBoxModelUI>({
            selected: false,
            name: faker.random.word(),
    ...
    describe('[Configure] Get Operations Success', () => {
        it('should insert original operations, operations and change needsUpdate to false  ', () => {
            const fakeOperationList: OperationModelUI[] = mockOperationModelUI.buildList(3);
            const action = new GetOperationsSuccess(fakeOperationList);
            const result = reducer(initialState, action);
            expect(result).toEqual({ ...initialState, operationsPage:
                {
    ...
    describe('[Configure] Get Operations SelectBox Success', () => {
        it('should insert/update operations selectbox', () => {
            const fakeOperationSelectBox: ConfigureSelectBoxModelUI[] = mockOperationSelectBoxModelUI.buildList();
            const action = new GetOperationsSelectBoxSuccess(fakeOperationSelectBox);
            const result = reducer(initialState, action);
            expect(result).toEqual(
                {
                    ...initialState, operationsSelectBox : [...fakeOperationSelectBox]
                }
            );
        });
    });
    ...
    describe('[Configure] Add Operation Success', () => {
        it('should update operations and operationsSelectBox array inserting a new entity', () => {
            ...
            expect(result).toEqual({
                ...
                {
                ... initialState.operationsPage,
                 operations: [fakeOperationToAdd, ...initialState.operationsPage.operations]
                },
                operationsSelectBox: [fakeNewElementForOperationsSelectBox, ...initialState.operationsSelectBox]
                });
        });
    });
    ...
    describe('[Configure] Remove Operation Success', () => {
        it('should update operations and operationsSelectBox array removing the entity desired', () => {
            ...
            const stateWithOperationsAndSelectBoxs: ConfigureState = {...initialState,
                operationsPage: {
            ...
            const expectedOperationsArray = fakeOperationsArray.filter((op) => op.Id !== 1);
            const expectedOperationsSelectBoxArray = fakeOperationSelectBoxArray.filter((op) => op.value !== 1);

            const result = reducer(stateWithOperationsAndSelectBoxs, action);
            expect(result).toEqual({ ...stateWithOperationsAndSelectBoxs,
                operationsPage:
                {
...
describe('Configure Reducer : Machine Operations State', () => {
    ...
    beforeEach(() => {
        mockMachineOperationModelUI = Factory.Sync.makeFactory<MachineOperationModelUI>({
            Id: Factory.each((i) => i),
    ...
    describe('[Configure] Get Machine Operations Success', () => {
        it('should update machines, originalMachines with action payload and needsUpdate must be set to false', () => {
            const mockedMachineOperationList: MachineOperationModelUI[] = mockMachineOperationModelUI.buildList(2);
            const action = new GetMachineOperationSuccess(mockedMachineOperationList);
            const result = reducer(initialState, action);

            expect(result).toEqual({ ...initialState, machinesPage:
                {
    ...
    describe('[Configure] Add Machine Operation', () => {
        it('should return the default state', () => {
            const mockedMachineOperationModel: MachineOperationModelUI = mockMachineOperationModelUI.build();
    ...
    describe('[Configure] Add Machine Operations Success', () => {
        it('should add a entity to machine array state', () => {
            const mockedMachineOperationModel: MachineOperationModelUI = mockMachineOperationModelUI.build();
            const action = new AddMachineOperationSuccess(mockedMachineOperationModel);
            const result = reducer(initialState, action);
            ...
            expect(result).toEqual({
                ...
                {
                ...initialState.machinesPage,
                machines: [mockedMachineOperationModel, ...initialState.machinesPage.machines]
                }
            }
            );
        });
    ...
    describe('[Configure] Remove Machine Operations Success', () => {
        it('should remove the entity in machine array state', () => {
            const mockMachineOperationList: MachineOperationModelUI[] = mockMachineOperationModelUI.buildList(2);
            const stateWithAddedMachineOperations: ConfigureState = { ...initialState,
                machinesPage: {
            ...
            const expectedMachineOperationsArrayAfterRemoval = mockMachineOperationList.filter((mop) => mop.Id !== 1 );
...
describe('Configure Reducer : Motes State', () => {
    ...
    describe('[Configure] Get Motes Success', () => {
        it('should update motes and original motes with the action payload and set needsUpdate to false', () => {
            ...
            expect(result).toEqual({ ...initialState, motesPage:
                {
    ...
    describe('[Configure] Get Motes SelectBox Success', () => {
        it('should update motesSelectBox', () => {
            const fakeMoteSelectBoxList: ConfigureSelectBoxModelUI[] = mockMoteSelectBoxUI.buildList(2);
            const action = new GetMotesSelectBoxSuccess(fakeMoteSelectBoxList);
            const result = reducer(initialState, action);

            expect(result).toEqual({
                ...initialState, motesSelectBox : fakeMoteSelectBoxList
            });
        });
    });
    ...
    describe('[Configure] Add Mote Success', () => {
        it('should add a mote  to motes array and motesSelectBox', () => {
            ...
            motesSelectBox: [...[fakeMoteSelectBoxModel], ...initialState.motesSelectBox]
            });
        });
    });
    ...
    describe('[Configure] Remove Mote Success', () => {
        it('should remove the mote by id, passed in the action payload, in motes and motesSelectBox', () => {
            ...
            const fakeMotesSelectBoxArray: ConfigureSelectBoxModelUI[] = fakeMotesArray.map((op) => {
            ...
            const stateWithMotesAndMotesSelectBox: ConfigureState = {...initialState,
                motesPage: {
            ...
            const expectedMotesSelectBoxArray = fakeMotesSelectBoxArray.filter((op) => op.value !== 1);

            const result = reducer(stateWithMotesAndMotesSelectBox, action);
            expect(result).toEqual({...stateWithMotesAndMotesSelectBox,
                motesPage:
                {
...
describe('Configure Reducer : Sensors State', () => {
    ...
    describe('[Configure] Get Sensors Success', () => {
        it('should update sensor and originalSensors with sensor list from action payload', () => {
            ...
            expect(result).toEqual({ ...initialState, sensorsPage:
                {
    ...
    describe('[Configure] Remove Sensor Success', () => {
        it('should return the default state', () => {
            ...
            const stateWithAddedSensors: ConfigureState = {...initialState,
                sensorsPage: {
                    needsUpdate: false,
            ...
            const expectedSensorsArrayAfterRemoval = fakeSensorList.filter((mop) => mop.Id !== 1 );
            expect(result).toEqual({
...
describe('Configure Reducer : EdgesSelectBox State, MachineSelectBox State, MessagesSelectBox State, SubcontractorSelectBox State', () => {
    ...
    describe('[Configure] Get Edges Select Box Success', () => {
        it('should update edgeSelectBox with a list of edge selectbox from the action payload', () => {
            ...
            expect(result).toEqual({
                ... initialState, edgeSelectBox: edgeFakeList
            });
        });
    });
    ...
    describe('[Configure] Get Subcontractors Select Box Success', () => {
        it('should update subcontractorsSelectBox with a list of subcontractors selectbox from the action payload', () => {
            ...
            const action = new GetSubcontractorsSelectBoxSuccess(subContractorFakeList);
            const result = reducer(initialState, action);

            expect(result).toEqual({
                ... initialState, subcontractorsSelectBox: subContractorFakeList
            });
        });
    });
    ...
    describe('[Configure] Get Messages Select Box Success', () => {
        it('should update messageSelectBox with a list of messages selectbox from the action payload', () => {
            ...
            expect(result).toEqual({
                ... initialState, messageSelectBox: messagesFakeList
            });
        });
    });
    ...
    describe('[Configure] Get Machines Select Box Success', () => {
        it('should update machineSelectBox with a list of machines selectbox from the action payload', () => {
            ...
            expect(result).toEqual({
                ... initialState, machineSelectBox: machinesFakeList
            });
        });
    });
});
describe('Configure Reducer : Resume Page State () => ', () => {
    ...
    it('should generate the resume page with the removed sensor', () => {
        const fakeOriginalSensorList: SensorModelUI[] = mockSensorModelUI.buildList(3);
        const fakeSensorList: SensorModelUI[] = fakeOriginalSensorList.filter((elem) => elem.Id !== 1);
        const removedSensor = {... fakeOriginalSensorList.filter((elem) => elem.Id === 1)[0]};
        const stateWithRemovedSensorAndOriginalSensors: ConfigureState = {... initialState,
            sensorsPage: {
                sensors: [...fakeSensorList],
        ...
        const result = reducer(stateWithRemovedSensorAndOriginalSensors, actionToGetResumePage);
        expect(result).toEqual({

...
export class ConfigureSensorsEffects {
  @Effect() public getSensors$ = this.actions$.pipe(
  ...
  @Effect() public addSensor$ = this.actions$.pipe(
    ofType<ConfigureActions>(ConfigurationActionTypes.AddSensor),
    tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
    switchMap((action) =>
      this.sensorService.AddEC(action.payload).pipe(
  ...
  @Effect() public removeSensor$ = this.actions$.pipe(

...
export class ConfigureProductEffects {
  @Effect() public getProductDetails$ = this.actions$.pipe(
  ...
  @Effect() public saveProductDetails$ = this.actions$.pipe(
    ...
    switchMap(([payload, payloadConvertedToDto]) =>
      this.productService.UpdateProduct(payloadConvertedToDto).pipe(
        map((_) => {

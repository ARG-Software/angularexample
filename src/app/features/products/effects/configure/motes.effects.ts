...
import { ConfigureActions, ConfigurationActionTypes,
     GetMotesSuccess, AddMoteSuccess, RemoveMoteSuccess, ErrorConfiguration, GetMotesSelectBoxSuccess } from '../../actions/configure.actions';
import { tap, withLatestFrom, filter, switchMap, map, concatMap, catchError, finalize } from 'rxjs/operators';
...
export class ConfigureMotesEffects {
    @Effect() public getMotes$ = this.actions$
    .pipe(
        ofType<ConfigureActions>(ConfigurationActionTypes.GetMotes),
        withLatestFrom(this.moduleStore$.pipe(select(fromModule.getMotesUpdateState))),
        filter(([_, updateNeeded]) => updateNeeded ),
        tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
        switchMap(([action, _]) =>
            this.moteService.GetMotesofProduct(action.payload)
                .pipe(
                    map((moteList: IMoteDto[]) => {
                        const moteListCasted = mapObjectTypeToRequested<MoteModelUI[]>(moteList);
                        ...
                        const actionsArray = [ new GetMotesSuccess(moteListCasted),
    ...
    @Effect() public addMote$ = this.actions$
    .pipe(
        ...
        switchMap((payload) =>
        this.moteService.AddMote(payload)
            .pipe(
   ...
   @Effect() public removeMote$ = this.actions$
   .pipe(
       ofType<ConfigureActions>(ConfigurationActionTypes.RemoveMote),
       tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
       switchMap((action) =>
       this.moteService.DeleteMote(action.payload)
           .pipe(

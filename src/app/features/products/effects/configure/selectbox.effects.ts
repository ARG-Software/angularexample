...
import { ConfigureActions, ConfigurationActionTypes,
    GetEdgesSelectBoxSuccess, GetMessagesSelectBoxSuccess,
    GetSubcontractorsSelectBoxSuccess,
    GetMachineSelectBoxSuccess,
    ErrorConfiguration } from '../../actions/configure.actions';
import { tap, withLatestFrom, filter, switchMap, map, finalize, catchError } from 'rxjs/operators';
import { MachineModelUI, ConfigureSelectBoxModelUI, SubcontractorModelUI, EdgeModelUI, MessageModelUI } from '../../models/configure.model';
import { Store, select } from '@ngrx/store';
import { of } from 'rxjs/observable/of';
import { IMachineService } from '@api/services/interfaces/core/imachine.service';
import { IMachineDto, ISubcontractorsDto, IEdgeDto, IContactMessageDto } from '@api/models/apimodels';
...
export class ConfigureSelectBoxEffects {
    @Effect() public getEdgesSelectBox$ = this.actions$
    .pipe(
         ...
         this.edgeService.GetEdges()
             .pipe(
                 map((edgeList: IEdgeDto[]) => {
                     const edgeListCasted = mapObjectTypeToRequested<EdgeModelUI[]>(edgeList);
    ...
    @Effect() public getMessagesSelectBox$ = this.actions$
    .pipe(
        ofType<ConfigureActions>(ConfigurationActionTypes.GetMessagesSelectBox),
        withLatestFrom(this.moduleStore$.pipe(select(fromModule.getMessagesSelectBox))),
        filter(([_, messagesSelectBox]) => messagesSelectBox.length === 0  ),
        tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
        switchMap(([__ , _]) =>
        this.messagesService.GetContactMessages()
            .pipe(
                map((messageList: IContactMessageDto[]) => {
                    const messageListCasted = mapObjectTypeToRequested<MessageModelUI[]>(messageList);
    ...
    @Effect() public getSubContractorsSelectBox$ = this.actions$
    .pipe(
            ofType<ConfigureActions>(ConfigurationActionTypes.GetSubcontractorsSelectBox),
            withLatestFrom(this.moduleStore$.pipe(select(fromModule.getSubContractorsSelectBox))),
            filter(([_, subcontractorsSelectBox]) => subcontractorsSelectBox.length === 0  ),
            tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
            switchMap(([__ , _]) =>
            this.subcontractorService.GetSubcontractors()
                .pipe(
                    map((machineList: ISubcontractorsDto[]) => {
                        const subcontractorsListCasted = mapObjectTypeToRequested<SubcontractorModelUI[]>(machineList);
                        const subcontractorsSelectBox = subcontractorsListCasted.map((element) => {
                            const newElement: ConfigureSelectBoxModelUI = {
                                name: element.Name,
                                selected: false,
                                value: element.Id
   ...
   @Effect() public getMachinesSelectBox$ = this.actions$
       .pipe(
            ofType<ConfigureActions>(ConfigurationActionTypes.GetMachineSelectBox),
            withLatestFrom(this.moduleStore$.pipe(select(fromModule.getMachineSelectBox))),
            ...
            this.machineService.GetMachines()
                .pipe(
                    map((machineList: IMachineDto[]) => {
                        const machineListCasted = mapObjectTypeToRequested<MachineModelUI[]>(machineList);

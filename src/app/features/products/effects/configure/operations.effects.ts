...
import { ConfigureActions, ConfigurationActionTypes, GetOperationsSuccess,
    AddOpertionSuccess, RemoveOperationSuccess, ErrorConfiguration, GetOperationsSelectBoxSuccess } from '../../actions/configure.actions';
import { tap, switchMap, map, finalize, catchError, filter, withLatestFrom, concatMap } from 'rxjs/operators';
import { OperationModelUI, ConfigureSelectBoxModelUI } from '../../models/configure.model';
...
export class ConfigureOperationsEffects {
        @Effect() public getOperations$ = this.actions$
            .pipe(
                ofType<ConfigureActions>(ConfigurationActionTypes.GetOperations),
                withLatestFrom(this.moduleStore$.pipe(select(fromModule.getOperationsUpdateState))),
                filter(([_, updateNeeded]) => updateNeeded ),
                tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
                switchMap(([action, _]) =>
                    this.operationService.GetOperationsByProductId(action.payload)
                        .pipe(
                            map((operationList: IOperationDto[]) => {
                                const operationListCasted = mapObjectTypeToRequested<OperationModelUI[]>(operationList);
                                ...
                                const actionsArray = [ new GetOperationsSuccess(operationListCasted),
    ...
    @Effect() public addOperation$ = this.actions$
     .pipe(
         ...
         switchMap((payload) =>
         this.operationService.AddOperation(payload)
             .pipe(
                 map((addedOperation: IOperationDto) => {
                     const operationCasted = mapObjectTypeToRequested<OperationModelUI>(addedOperation);
    ...
    @Effect() public removeOperation$ = this.actions$
    .pipe(
        ofType<ConfigureActions>(ConfigurationActionTypes.RemoveOperation),
        tap(() => this.mainStore$.dispatch(new loadingActions.ShowLoading())),
        switchMap((action) =>
        this.operationService.DeleteOperation(action.payload)
            .pipe(

...
import { MessagingLoadDataModelUI, MessagingRequestModelUI, MessagingSaveDataModelUI } from '../../models/messaging.model';
...
export class MessagingComponent implements OnInit, OnDestroy {
    ...
    public constructor(
        ...
        this._messagingToSave$ = this.store.select(fromReducer.getMessagingToSave).subscribe((toSave) =>
            this.messagingToSave = toSave
        );
    }
    ...
    private saveMessaging() {
        if (this.messagingToSave.length > 0) {
            this.store.dispatch(new fromActions.UpdateMessagingData(this.messagingToSave));
        }
    }
}

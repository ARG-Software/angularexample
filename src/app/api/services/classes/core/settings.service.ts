...
export class SettingsService extends BaseMimsApi implements ISettingsService {
    ...
    public UpdateWip( wipData: WipDataModelUI[] ): Observable<boolean> {
        return this.updateObject( wipData, `${this.controllerRoute}/updateBufferCount`);
    }

    public UpdateKanBan( kanbanData: KanbanDataModelUI[] ): Observable<boolean> {
        return this.updateObject( kanbanData, `${this.controllerRoute}/updateBufferKanBan`);
    }

    public UpdateDowntimeForProduct( productId: number, downtimeInMinutes: number ): Observable<boolean> {
        return this.updateObject({ productId, downtimeInMinutes }, `${this.controllerRoute}`);
    }
}

...
@Injectable()
export class MachineOperationService extends BaseMimsApi
  ...
  public AddMachineOperation(machineOperation: IMachineOperationsDto): Observable<IMachineOperationsDto> {
    return this.insertObject(machineOperation, `${this.controllerRoute}`);
  }
  public DeleteMachineOperation(machineOperationId: number): Observable<boolean> {
    return this.deleteById(machineOperationId, `${this.controllerRoute}`);
  }
  public GetMachineOperationsofProduct(productId: number): Observable<IMachineOperationsDto[]> {
